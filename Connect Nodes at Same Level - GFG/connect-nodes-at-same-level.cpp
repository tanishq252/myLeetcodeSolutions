...
class Solution{
    public:
    void traverseAndConnect(Node *root){
        if(root==NULL) return;
        ...
        traverseAndConnect(root->left);
        traverseAndConnect(root->right);
    }
    vector<vector<Node*>> v;
    void levelOrder(Node *root, int lvl){
        if(root==NULL) return;
        if(v.size() == lvl) v.push_back({});
        v[lvl].push_back(root);
